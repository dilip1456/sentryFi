...
const BudgetCell = ({ category, budget, onSave, onRemove }: {
  ...
  if (editing) return (
    <div className="flex items-center gap-1">
      ...
      <input autoFocus type="number" min={0} step={10} value={val}
        ...
        className="w-20 bg-secondary/40 border border-border/60 rounded px-1.5 py-0.5 text-[12.5px] text-foreground outline-none focus:border-[hsl(var(--primary)/0.5)]"
      />
  ...
  return (
    <button onClick={() => { setVal(String(budget ?? "")); setEditing(true); }}
      className="text-[12.5px] text-muted-foreground hover:text-foreground transition-colors text-right">
      {budget ? fmtUSD(budget) + "/mo" : <span className="text-[12px]">Set limit</span>}
    </button>
  );
};
      ...
      <div className={cn(
        ...
        <div className="flex border-b" style={{ borderColor: "var(--gold-border)" }}>
          <button onClick={() => setTab("categories")}
            className={cn("flex-1 py-2.5 text-[13px] font-medium transition-colors",
          ...
          <button onClick={() => setTab("unassigned")}
            className={cn("flex-1 py-2.5 text-[13px] font-medium transition-colors flex items-center justify-center gap-1.5",
              tab === "unassigned" ? "text-foreground border-b-2 border-[hsl(var(--primary))]" : "text-muted-foreground hover:text-foreground")}>
            Unassigned
            {unassigned.length > 0 && (
              <span className="px-1.5 py-0.5 rounded-full bg-warning/20 text-warning text-[12px] tabular">{unassigned.length}</span>
            )}
          </button>
        </div>
        ...
        {tab === "categories" && (
          <div className="flex-1 overflow-y-auto">
              ...
              {showNewCat ? (
                <div className="flex items-center gap-2">
                  <input autoFocus value={newCatName} onChange={e => setNewCatName(e.target.value)}
                    ...
                    className="flex-1 bg-secondary/40 border border-border/60 rounded-md px-3 py-1.5 text-[13px] text-foreground outline-none focus:border-[hsl(var(--primary)/0.5)]" />
                  <select value={newCatType} onChange={e => setNewCatType(e.target.value as "income"|"expense")}
                    className="bg-secondary/40 border border-border/60 rounded-md px-2 py-1.5 text-[13px] text-foreground outline-none">
            ...
            <div className="px-5 py-2">
              <div className="text-[12px] uppercase tracking-wider text-muted-foreground py-2">Expense</div>
                  ...
                  <div key={name} className="group flex items-center gap-3 py-2 border-b border-border/20 last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[13.5px] text-foreground">{name}</span>
                        {isCustom && <span className="text-[11px] px-1 py-0.5 rounded bg-secondary text-muted-foreground">custom</span>}
                      </div>
                      <div className="text-[12px] text-muted-foreground">{txnCount} transaction{txnCount !== 1 ? "s" : ""}</div>
                    </div>
            ...
            {/* Income categories */}
            <div className="px-5 py-2">
              <div className="text-[12px] uppercase tracking-wider text-muted-foreground py-2">Income</div>
            ...
            <div className="px-4 py-3 border-b border-border/30 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <button onClick={runAI} disabled={aiLoading || unassigned.length === 0}
                  className="flex-1 h-8 rounded-lg bg-gold text-[13px] font-medium hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-1.5">
                  {aiLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                  {aiLoading ? "Analyzing…" : "AI Suggest All"}
                </button>
                {Object.keys(aiSuggestions).length > 0 && (
                  <button onClick={acceptAllSuggestions}
                    className="h-8 px-3 rounded-lg border border-positive/40 bg-positive/10 text-positive text-[13px] font-medium hover:bg-positive/20 transition-colors flex items-center gap-1.5">
                ...
                {selected.size > 0 && (
                  <>
                    <select value={bulkCat} onChange={e => setBulkCat(e.target.value)}
                      className="flex-1 bg-secondary/40 border border-border/60 rounded-md px-2 py-1 text-[12.5px] text-foreground outline-none focus:border-[hsl(var(--primary)/0.5)]">
                    ...
                    <button onClick={applyBulk} disabled={!bulkCat}
                      className="h-7 px-3 rounded-md bg-gold text-[12.5px] font-medium hover:opacity-90 disabled:opacity-50">
                      Apply
                    </button>
                  </>
                )}
            ...
            <div className="flex-1 overflow-y-auto">
              {unassigned.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-6">
                  ...
                  <div className="text-[13px] text-muted-foreground">Every transaction has a category. Nice work!</div>
                </div>
              ) : (
                unassigned.map((t, i) => {
                    ...
                    <div key={t.id}
                      ...
                      {suggestion && (
                        <button onClick={() => acceptSuggestion(t.id)}
                          className="shrink-0 h-6 px-2 rounded bg-positive/15 text-positive text-[12px] font-medium hover:bg-positive/25 transition-colors flex items-center gap-1">
